  ...
  try {
    ...
    const allProducts = (data || []).map(row => {
      const specs = typeof row.specifications === "string"
        ...
        : (row.specifications || {});
      const cleanSlug = row.slug || specs.slug || String(row.name || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
      return {
        ...row,
        slug: cleanSlug,
        seo_title: row.seo_title || specs.seo_title || `${row.name} | Seven Spice`,
        meta_description: row.meta_description || specs.meta_description || row.description?.substring(0, 160) || "",
        primary_keyword: row.primary_keyword || specs.primary_keyword || "",
        secondary_keywords: row.secondary_keywords || specs.secondary_keywords || "",
...
export async function getProductById(idOrSlug) {
  ...
  const extractedId = separatorIndex !== -1 ? lower.slice(separatorIndex + 2) : null;

  const products = await getProductsRaw(true);
  const found = products.find(p => {
    const pId = (p.id || "").toLowerCase();
    const pSlug = (p.slug || p.specifications?.slug || "").toLowerCase().replace(/^\/products\//, "").replace(/^\//, "");
    const nameSlug = String(p.name || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    return (
      pId === lower ||
      pSlug === lower ||
      nameSlug === lower ||
      (extractedId && pId === extractedId)
    );
  });
  ...
  try {
    ...
    return {
      ...data,
      slug: data.slug || specs.slug || String(data.name || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, ""),
      seo_title: data.seo_title || specs.seo_title || `${data.name} | Seven Spice`,
      meta_description: data.meta_description || specs.meta_description || data.description?.substring(0, 160) || "",
      primary_keyword: data.primary_keyword || specs.primary_keyword || "",
      secondary_keywords: data.secondary_keywords || specs.secondary_keywords || "",
...
export async function saveProduct(product) {
  ...
  const existingSpecs = typeof product.specifications === "object"
    ? (product.specifications || {})
    : JSON.parse(product.specifications || "{}");

  const specs = {
    ...existingSpecs,
    slug: product.slug || existingSpecs.slug,
    seo_title: product.seo_title || existingSpecs.seo_title,
    meta_description: product.meta_description || existingSpecs.meta_description,
    primary_keyword: product.primary_keyword || existingSpecs.primary_keyword,
    secondary_keywords: product.secondary_keywords || existingSpecs.secondary_keywords,
  };
  ...
  return {
    ...prodToSave,
    slug: specs.slug || String(prodToSave.name || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, ""),
    seo_title: specs.seo_title || `${prodToSave.name} | Seven Spice`,
    meta_description: specs.meta_description || prodToSave.description?.substring(0, 160) || "",
    primary_keyword: specs.primary_keyword || "",
    secondary_keywords: specs.secondary_keywords || "",
  };
}
