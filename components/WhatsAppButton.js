...
export default function WhatsAppButton({ whatsappNumber, whatsappMessage, locale = "en" }) {
  ...
  return (
    <>
      ...
      <div
        className={`fixed bottom-5 sm:bottom-6 md:bottom-8 z-50 flex items-center gap-2 sm:gap-3 group transition-all duration-300 ${alignmentClass}`}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        {/* Tooltip Label Container */}
        <div
          className={`hidden xs:block bg-surface/95 backdrop-blur-md border border-on-surface/10 text-on-surface font-sans text-xs sm:text-xs md:text-sm font-semibold tracking-wide py-2.5 px-3.5 sm:py-2.5 sm:px-4 rounded-xl shadow-xl transition-all duration-300 select-none whitespace-nowrap max-w-[170px] sm:max-w-none truncate
        ...
        <a
          ...
          aria-label={ctaText}
          className="whatsapp-pulse-button w-14 h-14 sm:w-15 sm:h-15 md:w-16 md:h-16 rounded-full flex items-center justify-center bg-[#25D366] text-white shadow-2xl cursor-pointer hover:bg-[#20ba5a] hover:scale-105 active:scale-95 transition-all duration-300 shrink-0 border-2 border-white/20"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 448 512"
            className="w-7 h-7 sm:w-7 sm:h-7 md:w-8 md:h-8 fill-current drop-shadow-sm"
          >
